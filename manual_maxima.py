# -*- coding: utf-8 -*-
"""
Second attempt: Finding local maxima in protein signal
"""
...
from skimage import data, measure, exposure, img_as_ubyte
...
def find_local_maxima_np(img_data):
    ...
    max_out = ndimage.maximum_filter(img_data,size=3)
    #Find local maxima.
...
nuclei = load_file(files[0], 0)
protein = load_file(files[0], 1)
...
# conversion to 8bit
img_data = img_as_ubyte(exposure.rescale_intensity(protein))
plt.imshow(protein)
plt.imshow(img_data)

ntol = 20 #Noise Tolerance.
...
points = np.stack((ypts, xpts), axis=1)
